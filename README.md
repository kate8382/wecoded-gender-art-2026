...
- Проблема: дискретный зум казался «мягким» и запаздывал относительно музыки.
	- Решение: добавлен snap‑режим (ступенчатый зум) на RAF, плюс параметры `leadSeconds` и `leadFirstSeconds` в `zoomTimeline`, чтобы опережать первые шаги (по умолчанию 0.1s). Это даёт более «резкий» визуальный отклик.
